...
class NodeRtmpSession {
  ...
  onPublish(invokeMessage) {
    ...
    this.liveId = invokeMessage.streamName.split('?')[0];
    this.publishStreamPath = '/' + this.appname + '/' + this.liveId;
    ...
    if (!this.isIPC) {
      context.nodeEvent.emit('prePublish', this.id, this.publishStreamPath, this.publishArgs, this.liveId);
    }
    if (!this.isStarting) {
      return;
    }
    ...
    if (context.publishers.has(this.publishStreamPath)) {
      ...
      setTimeout(() => {
        ...
        if (!this.isIPC) {
          context.nodeEvent.emit('postPublish', this.id, this.publishStreamPath, this.publishArgs, this.liveId);
        }
    ...
    if (invokeMessage.streamId == this.publishStreamId) {
      if (this.isPublishing) {
        if (!this.isIPC) {
          ...
          context.nodeEvent.emit('donePublish', this.id, this.publishStreamPath, this.publishArgs, this.liveId);
        }
        if (this.isStarting) {
